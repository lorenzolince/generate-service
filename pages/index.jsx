...
const Index = () => {
    ...
    const [appicationSources, setAppicationSources] = useState(null);
    ...
    const [modePrepaid, setModePrepaid] = useState(false);
    ...
    const resulsetSp = async (e, index, data) => {
        swal.fire({
            title: t('index:titleAlertResulset'),
            text: t('index:textAlertResulset'),
            icon: "success",
            input: 'text',
            buttons: true,
    ...
    const onSubmitShema = async (data) => {
        await generateDataApp(data, "JSON_XSD")
    }
            ...
            <Container>
                <Form>
                    ...
                    <Row>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            <Form.Group>
                                <Form.Label htmlFor="domain" >{t('index:domain')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                appicationServer ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'applicationService'} >{t('index:applicationService')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            <Form.Group>
                                <Form.Label htmlFor={'typeDataBase'} >{t('index:typeDataBase')}</Form.Label>
                                <Form.Control name="typeDataBase"  {...register("typeDataBase", { onChange: (e) => onChangeDatabase(e.target.value) })} id="typeDataBase" className={`form-control ${errors.typeDataBase ? 'is-invalid' : ''}`} as="select">
                                    {
                                        dataBaseType != null && dataBaseType.map((item) =>
                                            <option key={item.id} value={item.description}> {item.description} </option>
                                        )
                                    }
                                </Form.Control>
                    ...
                    <Row>
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                datasourceDropDown ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'dataSources'} >{t('index:dataSources')}</Form.Label>
                        ...
                        {(appServer != "EMBEDDED" && appServer != "TOMCAT" && dataBaseType != null) &&
                            <Col md={{ span: 2, offset: 1 }}>
                                ...
                                <Form.Group>
                                    <Form.Label htmlFor="jndi" >{t('index:jndi')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            {
                                schemas ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'schema'} >{t('index:schema')}</Form.Label>
                        ...
                        {dataProcedures && sources === "DATABASE" &&
                            <Col md={{ span: 2, offset: 1 }}>
                                <Form.Group>
                                    <Form.Label htmlFor={'procedures'} >{t('index:procedures')}</Form.Label>
                            ...
                            sources === "API_SOURCES" ?
                                <Col>
                                    <ApiSourcesGrid
                                        ...
                                        hiddenColumns={["typeMethod", "parameterType", "schema", "host", "port", "sender", "fileInput", "saveFile", "isProducer", "isConsumer", "header", "senderType"]}
                                        pagination={true}
                                ...
                                <Col md={{ span: 0, offset: 0 }}>
                                    <Button className="common-button-color" type="button" disabled={formState.isSubmitting} onClick={
                                        async () => {
                                            ...
                                            swal.fire({
                                                title: t('index:titleAlertDevops'),
