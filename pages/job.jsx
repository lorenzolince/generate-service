...
const Job = () => {
    ...
    const [columnsHiden, setColumnsHiden] = useState(["name", "type", "ResulsetValue", "senderEndpoint", "senderNameRequest", "senderPath", "senderQueryOrMutation", "authType"]);
    ...
    const generateSchema = Yup.object().shape({
        ...
        typeDataBase: (sources === "DATABASE" || jobType === "QUARTZ_DB") ? Yup.string()
            .required(t('common:valRequiredField').replace("[field]", t('job:typeDataBase'))) : null
    ...
    const onClicAdd = async (row) => {
        ...
        if (!isRepeated) {
            ...
            newRow.forEach(function (e, i) {
                e.sender = <SenderType item={e} producerWithSender={producerWith} currenQuue={queue} rowIndex={i} addOutSenderQueue={addOutSenderQueueSources} domain={getValues("domain")}></SenderType>;
            })

            setSourcesData(newRow)
        }
    }
    const addOutSenderQueueSources = async (rowIndex, item) => {
    ...
    const onChangeSources = async (value) => {
        setSources(value)
        if (getValues("jobType") !== "QUARTZ_DB") {
    ...
    const generateProcedure = async (procedure) => {
        try {
            ...
            if (typeDataBase == "SQL_SERVER" || typeDataBase == "MY_SQL") {
                setColumnsHiden(["id", "name", "type", "owner", "packNme", "ResulsetValue", "senderEndpoint", "senderNameRequest", "senderPath", "senderQueryOrMutation", "authType"])
            } else if (typeDataBase == "POSTGRES") {
                setColumnsHiden(["name", "type", "owner", "packNme", "ResulsetValue", "Resulset", "senderEndpoint", "senderNameRequest", "senderPath", "senderQueryOrMutation", "authType"])
            } else {
                setColumnsHiden(["id", "name", "ResulsetValue", "Resulset", "senderEndpoint", "senderNameRequest", "senderPath", "senderQueryOrMutation", "authType"])
            }
    ...
    const resulsetSp = async (e, index, data) => {
        swal.fire({
            title: t('job:titleAlertResulset'),
            text: t('job:textAlertResulset'),
            icon: "success",
            input: "text",
            buttons: true,
    ...
    const onSubmit = async (data) => {
        await generateDataApp(data, "JOB")
    }
    ...
    const generateDataApp = async (data, schema) => {
        let app = []
        let sps = []
        let jobTypeQuartz = data.jobType === "QUARTZ_DB" ? true : false
        proceduresGenerate.forEach(function (item) {
            sps.push({
                ...
                "producer": jobTypeQuartz,
                "consumer": true,
                "senderNameRequest": item.senderNameRequest,
                "senderPath": item.senderPath,
                "senderEndpoint": item.senderEndpoint,
                "senderQueryOrMutation": item.senderQueryOrMutation,
                "senderType": item.senderType === "QUEUE" ? "RABBIT_MQ" : item.senderType,
                "authType": item.authType
            })
        })
        sourcesData.forEach(function (item) {
            app.push({
                ...
                "senderEndpoint": item.senderEndpoint,
                "senderQueryOrMutation": item.senderQueryOrMutation,
                "senderType": item.senderType === "QUEUE" ? "RABBIT_MQ" : item.senderType ? item.senderType : "",
                "authType": item.authType,
        ...
        let datagenerate = {
            ...
            "producerServiceWith": "JOB",
            "typeServiceQueue": data.queueService,
            "auth": data.auth,
            "sourceType": sources,
            "jobTypeQuartz": jobTypeQuartz,
            "test": test

        }
        let generateDocument = {};
        if (schema === "DEVOPS") {
            datagenerate.typeSevice = "JOB";
    ...
    const reloadData = async () => {
        ...
        producerServiceWith.push({ id: "ACTIVE_MQ", description: "ACTIVE_MQ" });
        producerServiceWith.push({ id: "KAFKA", description: "KAFKA" });
                ...
                <Form>
                    ...
                    <Row>
                        <Col md={{ span: 2, offset: 0 }}>
                            <Form.Group>
                                <Form.Label htmlFor="name" >{t('job:name')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            <Form.Group>
                                <Form.Label htmlFor="domain" >{t('job:domain')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            {
                                queueService ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'jobType'} >{t('job:jobType')}</Form.Label>
                                        <Form.Control name="queueService"  {...register("jobType", { onChange: (e) => setJobType(e.target.value) })} id="jobType" className={`form-control ${errors.jobType ? 'is-invalid' : ''}`} as="select">
                                            {
                                                ["SCHEDULED", "QUARTZ_DB"].map((item) =>
                    ...
                    <Row>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                dataBaseType ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'typeDataBase'} >{t('job:typeDataBase')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            {
                                datasourceDropDown ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'dataSources'} >{t('job:dataSources')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            {
                                schemas ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'schema'} >{t('job:schema')}</Form.Label>
                        ...
                        {dataProcedures &&
                            <Col md={{ span: 2, offset: 0 }}>
                                <Form.Group>
                                    <Form.Label htmlFor={'procedures'} >{t('job:procedures')}</Form.Label>
                    ...
                    <Row>
                        {
                            proceduresGenerate.length > 0 && sources === "DATABASE" ?
                                <Col>
                                    <DataGrid data={proceduresGenerate} schemaColumns="job" hiddenColumns={columnsHiden} pagination={true} queue={false} />
                                </Col>
                                : null
                            ...
                            sources === "API_SOURCES" ?
                                <Col>
                                    <ApiSourcesGrid
                                        ...
                                        hiddenColumns={["typeMethod", "parameterType", "schema", "host", "port", "fileInput", "saveFile", "isProducer", "isConsumer", "headers"]}
                                        pagination={true}
