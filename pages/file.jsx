...
const File = () => {
    ...
    const onClicAdd = async (row) => {
        ...
        if (!isRepeated) {
            ...
            newRow.forEach(function (item, i) {
                ...
                item.saveFile = <Form.Control onChange={(e) => item.SenderType = e.target.value} as="select">
                    {
                        [{ id: "", description: t('common:select') },
    ...
    const generateProcedure = async (procedure) => {
        try {
            ...
            inputProcedure.forEach(function (item, i) {
                item.ResulsetValue = 0
                if (item.type === "P" || item.type === "PROCEDURE") {
                    item.Resulset = <Link href="#" onClick={(e) => resulsetSp(e, i, inputProcedure)} variant="danger" size="sm" >{item.ResulsetValue}</Link>
                } else {
                    item.Resulset = ""
                }
                item.inputFile = "JSON"
                ...
                item.Delete = <Button onClick={(e) => deleteSp(e, i, inputProcedure)} variant="danger" size="sm" >{t('common:btnDelete')}</Button>
            })

            if (typeDataBase == "SQL_SERVER" || typeDataBase == "MY_SQL") {
                setColumnsHiden(["id", "name", "type", "owner", "packNme", "ResulsetValue", "SenderType", "inputFile", "senderType"])
            } else if (typeDataBase == "POSTGRES") {
                setColumnsHiden(["name", "type", "owner", "packNme", "ResulsetValue", "Resulset", "SenderType", "inputFile", "senderType"])
            } else {
                setColumnsHiden(["id", "name", "ResulsetValue", "Resulset", "SenderType", "inputFile", "senderType"])
            }
    ...
    const resulsetSp = async (e, index, data) => {
        swal.fire({
            title: t('file:titleAlertResulset'),
            text: t('file:textAlertResulset'),
            icon: "success",
            input: "text",
            buttons: true,
            ...
            if (result.isConfirmed) {
                ...
                procedure.forEach(function (item, i) {
                    if (item.type === "P" || item.type === "PROCEDURE") {
                        item.Resulset = <Link href="#" onClick={(e) => resulsetSp(e, i, procedure)} variant="danger" size="sm" >{item.ResulsetValue}</Link>
                    } else {
                        item.Resulset = ""
                    }
                    item.inputFile = "JSON"
                    ...
                    item.Delete = <Button onClick={(e) => deleteSp(e, i, procedure)} variant="danger" size="sm" >{t('common:btnDelete')}</Button>
    ...
    const generateDataApp = async (data, schema) => {
        ...
        proceduresGenerate.forEach(function (item) {
            sps.push({
                ...
                "senderType": item.SenderType ? item.SenderType : "",
                ...
                <Form>
                    ...
                    <Row>
                        <Col md={{ span: 2, offset: 0 }}>
                            <Form.Group>
                                <Form.Label htmlFor="name" >{t('file:name')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            <Form.Group>
                                <Form.Label htmlFor="domain" >{t('file:domain')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                appicationServer ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'applicationService'} >{t('file:applicationService')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            ...
                            <Form.Group>
                                ...
                                <Form.Control name="auth"  {...register("auth", { onChange: (e) => setValue("auth", e.target.value) })} id="auth" className={`form-control ${errors.auth ? 'is-invalid' : ''}`} as="select">
                                    {
                                        [{ id: false, description: "LOCAL_FILE" }, { id: true, description: "SMB_SHAREFILE" }].map((item) =>
                                            <option key={item.id} value={item.id}> {item.description} </option>
                                        )
                                    }
                                </Form.Control>
                    ...
                    <Row>
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                dataBaseType ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'typeDataBase'} >{t('file:typeDataBase')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                datasourceDropDown ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'dataSources'} >{t('file:dataSources')}</Form.Label>
                        ...
                        {(appServer != "EMBEDDED" && appServer != "TOMCAT" && dataBaseType != null) &&
                            <Col md={{ span: 2, offset: 1 }}>
                                ...
                                <Form.Group>
                                    <Form.Label htmlFor="jndi" >{t('file:jndi')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            {
                                schemas ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'schema'} >{t('file:schema')}</Form.Label>
                        ...
                        {dataProcedures &&
                            <Col md={{ span: 2, offset: 1 }}>
                                <Form.Group>
                                    <Form.Label htmlFor={'procedures'} >{t('file:procedures')}</Form.Label>
                    ...
                    <Row>
                        {
                            proceduresGenerate.length > 0 && sources === "DATABASE" ?
                                <Col>
                                    <DataGrid data={proceduresGenerate} schemaColumns="sftp" hiddenColumns={columnsHiden} pagination={true} />
                                </Col>
                                : null
                            ...
                            sources === "API_SOURCES" ?
                                <Col>
                                    <ApiSourcesGrid
                                        ...
                                        hiddenColumns={["typeMethod", "parameterType", "schema", "host", "port", "sender", "isProducer", "isConsumer", "headers", "senderType"]}
                                        pagination={true}
                                        apiTypeList={[{ id: "REST_FULL", description: "REST_FUL" }, { id: "GRAPHQL", description: "GRAPHQL" }, { id: "SOAP", description: "SOAP" }, { id: "GRPC", description: "GRPC" }]}
