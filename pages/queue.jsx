...
import SenderType from "../components/SenderType"
...
const Queue = () => {
    ...
    const [queue, setQueue] = useState("RABBIT_MQ");
    const [producerWith, setProducerWith] = useState(null);
    ...
    const [columnsHiden, setColumnsHiden] = useState(["name", "type", "ResulsetValue", "senderEndpoint", "senderNameRequest", "senderPath", "senderQueryOrMutation", "inputFile", "authType"]);
    const [columnsHidenSrc, setColumnsHidenSrc] = useState(["typeMethod", "parameterType", "schema", "host", "port", "inputFile", "saveFile", "headers"]);
    ...
    const onClicAdd = async (row) => {
        ...
        if (!isRepeated) {
            ...
            newRow.forEach(function (item, i) {
                item.isProducer = <Form.Check type="checkbox" className="queue-center-text" id={`is-producer-switch-${i}`} onChange={(e) => item.producer = e.target.checked} />
                item.isConsumer = <Form.Check type="checkbox" className="queue-center-text" id={`is-consumer-switch-${i}`} onChange={(e) => item.consumer = e.target.checked} />
                item.Delete = <Button onClick={(e) => onClicDelete(newRow, i)} variant="danger" size="sm" >{t('common:btnDelete')}</Button>
                if (getValues("producerWith") === "FILE" || getValues("producerWith") === "SFTP") {
                    item.fileInput = <Form.Control onChange={(e) => item.inputFile = e.target.value} as="select">
                        {
                            [{ id: "JSON", description: "JSON" },
                            { id: "CSV", description: "CSV" },
                            { id: "XML", description: "XML" },
                            ].map((item) =>
                ...
                item.sender = <SenderType item={item} producerWithSender={producerWith} currenQuue={queue} rowIndex={i} addOutSenderQueue={addOutSenderQueueSrc} domain={getValues("domain")}></SenderType>;
            })
            setSourcesData(newRow)
        }
    }
    const addOutSenderQueueSrc = async (rowIndex, item) => {
        ...
        setSourcesData(old =>
            old.map((row, index) => {
                if (index === rowIndex) {
                    return {
                        ...old[rowIndex],
                        ["senderEndpoint"]: item.senderEndpoint,
                        ["senderNameRequest"]: item.senderNameRequest,
                        ["senderPath"]: item.senderPath,
                        ["senderQueryOrMutation"]: item.senderQueryOrMutation,
    ...
    const addOutSenderQueue = async (rowIndex, item) => {

        setProceduresGenerate(old =>
            old.map((row, index) => {
                if (index === rowIndex) {
                    return {
                        ...old[rowIndex],
    ...
    const onChangeQueue = async (e, rowIndex, columnName, value) => {

        setProceduresGenerate(old =>
            old.map((row, index) => {
                if (index === rowIndex) {
                    if (columnName === "consumer" && value == false) {
                        return {
                            ...old[rowIndex],
                            [columnName]: value,
                            ["sender"]: value,
                    ...
                    } else if (columnName === "consumer" && value == true) {
                        old[rowIndex][columnName] = value
                        const senderValue = old[rowIndex]["sender"] = <SenderType item={old[rowIndex]} producerWithSender={producerWith} currenQuue={queue} rowIndex={rowIndex} addOutSenderQueue={addOutSenderQueue} domain={getValues("domain")}></SenderType>
                        return {
                            ...old[rowIndex],
                            [columnName]: value,
                            ["sender"]: senderValue,
    ...
    const generateProcedure = async (procedure) => {
        try {
            ...
            inputProcedure.forEach(function (e, i) {
                ...
                e.consumer = true;
                if (getValues("producerWith") === "FILE" || getValues("producerWith") === "SFTP") {
                    e.fileInput = <Form.Control onChange={(ei) => e.inputFile = ei.target.value} as="select">
                        {
                            [{ id: "JSON", description: "JSON" },
                            { id: "CSV", description: "CSV" },
                            { id: "XML", description: "XML" },
                ...
                e.sender = <SenderType item={e} producerWithSender={producerWith} currenQuue={queue} rowIndex={i} addOutSenderQueue={addOutSenderQueue} domain={getValues("domain")}></SenderType>;
            ...
            if (typeDataBase == "SQL_SERVER" || typeDataBase == "MY_SQL") {
                setColumnsHiden(["id", "name", "type", "owner", "packNme", "ResulsetValue", "senderEndpoint", "senderNameRequest", "inputFile", "senderPath", "senderQueryOrMutation", "authType"])
            } else if (typeDataBase == "POSTGRES") {
                setColumnsHiden(["name", "type", "owner", "packNme", "ResulsetValue", "Resulset", "senderEndpoint", "senderNameRequest", "inputFile", "senderPath", "senderQueryOrMutation", "authType"])
            } else {
                setColumnsHiden(["id", "name", "ResulsetValue", "Resulset", "senderEndpoint", "senderNameRequest", "senderPath", "inputFile", "senderQueryOrMutation", "authType"])
            }
    ...
    const resulsetSp = async (e, index, data) => {
        swal.fire({
            title: t('queue:titleAlertResulset'),
            text: t('queue:textAlertResulset'),
            icon: "success",
            input: "text",
            buttons: true,
    ...
    const getTypeQueueService = async () => {
        try {
            const response = await typesResources.getTypeQueueService();
            return response
        }
        catch (error) {
        }
    }
    const getProducerServiceWith = async () => {
        try {
            const response = await typesResources.getProducerServiceWith();
            return response
        }
        catch (error) {
        }
    ...
    const onChangeDropDownQueue = async (valueQueue) => {
        setValue("queueService", valueQueue)
        setQueue(valueQueue)
        proceduresGenerate.forEach(function (item, rowIndex) {
            const columnName = "consumer";
            const value = item.consumer;
            setProceduresGenerate(old =>
                old.map((row, index) => {
                    if (index === rowIndex) {
                        if (columnName === "consumer" && value == false) {
                            ...
                            old[rowIndex][columnName] = value
                            const senderValue = old[rowIndex]["sender"] = <SenderType item={old[rowIndex]} producerWithSender={producerWith} currenQuue={valueQueue} rowIndex={rowIndex} addOutSenderQueue={addOutSenderQueue} domain={getValues("domain")}></SenderType>
                            return {
                                ...old[rowIndex],
                                [columnName]: value,
                                ["sender"]: senderValue,
        ...
        sourcesData.forEach(function (item, rowIndex) {
            const columnName = "consumer";
            const value = item.consumer;
            setSourcesData(old =>
                old.map((row, index) => {
                    if (index === rowIndex) {
                        if (columnName === "consumer" && value == false) {
                            ...
                            old[rowIndex][columnName] = value
                            const senderValue = old[rowIndex]["sender"] = <SenderType item={old[rowIndex]} producerWithSender={producerWith} currenQuue={valueQueue} rowIndex={rowIndex} addOutSenderQueue={addOutSenderQueueSrc} domain={getValues("domain")}></SenderType>
                            return {
                                ...old[rowIndex],
                                [columnName]: value,
                                ["sender"]: senderValue,
 ...
 */
    ...
    const onSubmit = async (data) => {
        await generateDataApp(data, "QUEUE")
    }
    ...
    const generateDataApp = async (data, schema) => {
        let app = []
        let sps = []
        let isProducerConsumer = true
        console.log("proceduresGenerate", proceduresGenerate)
        console.log("sourcesData", sourcesData)
        proceduresGenerate.forEach(function (item) {
            if (!item.producer && !item.consumer) {
            ...
            sps.push({
                ...
                "senderType": item.senderType,
                "authType": item.authType,
                "typeInput": item.inputFile ? item.inputFile : "JSON"
            })
        })
        ...
        if (!isProducerConsumer) {
            swal.fire({
                title: t('queue:titleAlertProducerConsumer'),
                text: t('queue:textAlertProducerConsumer'),
                icon: "warning",
                dangerMode: true,
                buttons: false,
                timer: 3500
        ...
        let datagenerate = {
            ...
            "producerServiceWith": data.producerWith,
            "typeServiceQueue": data.queueService,
            "auth": data.auth,
            "sourceType": sources,
            "test": test
        }
        ...
        if (schema === "DEVOPS") {
            ...
            if (generateDocument.status == 200) {
                reloadData();
                reset({ domain: "", name: "", appicationSources: "DATABASE", typeDataBase: "", dataSources: "", schema: "" });
                swal.fire({
    ...
    const fetchServices = async () => {
        return Promise.all([
            ...
            getTypeDataBase(),
            getTypeQueueService(),
            getProducerServiceWith(),
            getAppicationSources()
        ]).then(([data, dataBase, queueService, producer, sources]) => {
            return { data, dataBase, queueService, producer, sources };
        });
    }
    ...
    const reloadData = async () => {
        ...
        appServer.queueService.forEach(function (item, index) {
            queueService.push({ id: index, description: item });
        });
        appServer.producer.forEach(function (item, index) {
            producerServiceWith.push({ id: index, description: item });
        });

        setQueueService(queueService)
        setProducerWith(producerServiceWith)
                ...
                <Form>
                    ...
                    <Row>
                        <Col md={{ span: 2, offset: 0 }}>
                            <Form.Group>
                                <Form.Label htmlFor="name" >{t('queue:name')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            <Form.Group>
                                <Form.Label htmlFor="domain" >{t('queue:domain')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                queueService ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'queueService'} >{t('queue:applicationQueue')}</Form.Label>
                                        <Form.Control name="queueService"  {...register("queueService", { onChange: (e) => onChangeDropDownQueue(e.target.value) })} id="queueService" className={`form-control ${errors.queueService ? 'is-invalid' : ''}`} as="select">
                                            {
                                                queueService.map((item) =>
                                                    <option key={item.id} value={item.description}> {item.description} </option>
                                                )
                                            }
                                        </Form.Control>
                                        <div className="invalid-feedback">{errors.queueService?.message}</div>
                                    </Form.Group>
                                    : null
                            }
                        </Col>
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                producerWith ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'producerWith'} >{t('queue:applicationType')}</Form.Label>
                                        <Form.Control name="producerWith"  {...register("producerWith", { onChange: (e) => { setValue("producerWith", e.target.value); setAppicationServe(e.target.value) } })} id="producerWith" className={`form-control ${errors.producerWith ? 'is-invalid' : ''}`} as="select">
                                            {
                                                producerWith.map((item) =>
                                                    <option key={item.id} value={item.description}> {item.description} </option>
                                                )
                                            }
                                        </Form.Control>
                                        <div className="invalid-feedback">{errors.producerWith?.message}</div>
                                    </Form.Group>
                                    : null
                            }
                        </Col>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                appicationServer === "SFTP" &&
                                <Form.Group>
                                    <Form.Label htmlFor={'auth'} >{t('sftp:auth')}</Form.Label>
                                    <Form.Control name="auth"  {...register("auth", { onChange: (e) => setValue("auth", e.target.value) })} id="auth" className={`form-control ${errors.auth ? 'is-invalid' : ''}`} as="select">
                                        {
                                            [{ id: true, description: "AUTH_BASIC" }, { id: false, description: "AUTH_RSA_KEY" }].map((item) =>
                                                <option key={item.id} value={item.id}> {item.description} </option>
                                            )
                                        }
                                    </Form.Control>
                                    <div className="invalid-feedback">{errors.auth?.message}</div>
                                </Form.Group>
                            }
                        </Col>
                    </Row>
                    ...
                    <Row>
                        ...
                        <Col md={{ span: 2, offset: 0 }}>
                            {
                                dataBaseType ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'typeDataBase'} >{t('queue:typeDataBase')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            {
                                datasourceDropDown ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'dataSources'} >{t('queue:dataSources')}</Form.Label>
                        ...
                        <Col md={{ span: 2, offset: 1 }}>
                            {
                                schemas ?
                                    <Form.Group>
                                        <Form.Label htmlFor={'schema'} >{t('queue:schema')}</Form.Label>
                        ...
                        {dataProcedures &&
                            <Col md={{ span: 2, offset: 1 }}>
                                <Form.Group>
                                    <Form.Label htmlFor={'procedures'} >{t('queue:procedures')}</Form.Label>
                    ...
                    <Row>
                        {
                            proceduresGenerate.length > 0 && sources === "DATABASE" ?
                                <Col>
                                    <DataGrid data={proceduresGenerate} schemaColumns="queue" hiddenColumns={columnsHiden} pagination={true} onChangeQueue={onChangeQueue} queue={true} />
                                </Col>
                                : null
                                ...
                                <Col md={{ span: 2, offset: 1 }}>
                                    <Button className="common-button-color" type="button" disabled={formState.isSubmitting} onClick={
                                        async () => {
                                            ...
                                            swal.fire({
                                                title: t('queue:titleAlertDevops'),
